...
public class FlowerStore extends Bouquet implements Serializable {
    ...
    private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
        this.format = null;
        String line = stream.readLine();    //Из первой строки узнаем какой формат кодировки был использован ранее
        if (line != null) {
        ...
        if (this.format != null) {
            switch (format) {
                case XML: {
                    ...
                    this.parseXML(stream);  //входной поток уходит без первой строки - <?xml version="1.1" encoding="UTF-8" ?>
                }
                break;
                case JSON: {
                    this.setDataFormat(DataFormat.JSON);
                    this.parseJSON(stream);  //входной поток уходит без первой строки - {
                }
                break;
            }
        ...
        if (this.flowerStacks != null) {
            ...
            for (int i = 0; i < this.flowerStacks.length; i++) {
                if (this.flowerStacks[i] != null) {
                    out.writeChars("\t\t{\n");
                    out.writeObject(this.flowerStacks[i]);
                    out.writeChars("\t\t}");
            ...
            if (!line.equals("]")) {  //Если массив стэков не пустой
                ...
                while (!(line = in.readLine().trim()).equals("]")) {    //Если новый виток цикла начинается с символа "]" - это конец массива
                    ...
                    if (line.equals("{")) {
                        ...
                        this.flowerStacks[index] = (FlowerStack)in.readObject();
                        ...
                        if (line.indexOf(caption) >= 0) {
                            ...
                            if(line.indexOf(caption) == -1){   //Если элемент массива стеков не пустой
                                ...
                                if(!line.equals("</flowerStack>")){
                                    throw new IOException("Missing </flowerStack> in XML-object");
                                }
                            }
                        }else{
